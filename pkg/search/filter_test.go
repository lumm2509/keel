...
func TestFilterDataBuildExpr(t *testing.T) {
	...
	for _, s := range scenarios {
		t.Run(s.name, func(t *testing.T) {
			...
			rawSql := expr.Build(&search.BuildContext{}, search.Params{})
...
func TestFilterDataBuildExprWithParams(t *testing.T) {
	...
	gotQuery := search.NewDB().Select("*").Distinct(false).Where(expr).Build().SQL()
	...
	expectedParts := []string{
		...
		"`test4` IS NULL",
		"`test6` = 'simple'",
		"`test10` = '2023-01-01 00:00:00 +0000 UTC'",
	}
...
func TestLikeParamsWrapping(t *testing.T) {
	...
	expectedParts := []string{
		"SELECT * WHERE",
		"`test1` LIKE '%abc%' ESCAPE '\\'",
		"`test2` LIKE 'ab%c' ESCAPE '\\'",
		"`test7` LIKE",
		"`test12` LIKE",
	}
	for _, part := range expectedParts {
