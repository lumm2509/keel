...
func resolveTokenizedExpr(expr fexpr.Expr, fieldResolver FieldResolver) (Expression, error) {
...
func buildResolversExpr(
	...
	case fexpr.SignLike, fexpr.SignAnyLike:
		...
		if len(right.Params) == 0 {
			expr = NewExp(fmt.Sprintf("%s LIKE ('%%' || %s || '%%') ESCAPE '\\'", left.Identifier, right.Identifier), left.Params)
		} else {
			expr = NewExp(fmt.Sprintf("%s LIKE %s ESCAPE '\\'", left.Identifier, right.Identifier), mergeParams(left.Params, wrapLikeParams(right.Params)))
		}
	case fexpr.SignNlike, fexpr.SignAnyNlike:
		...
		if len(right.Params) == 0 {
			expr = NewExp(fmt.Sprintf("%s NOT LIKE ('%%' || %s || '%%') ESCAPE '\\'", left.Identifier, right.Identifier), left.Params)
		} else {
			expr = NewExp(fmt.Sprintf("%s NOT LIKE %s ESCAPE '\\'", left.Identifier, right.Identifier), mergeParams(left.Params, wrapLikeParams(right.Params)))
		}
	case fexpr.SignLt, fexpr.SignAnyLt:
		expr = NewExp(fmt.Sprintf("%s < %s", left.Identifier, right.Identifier), mergeParams(left.Params, right.Params))
...
func resolveToken(token fexpr.Token, fieldResolver FieldResolver) (*ResolverResult, error) {
	...
	case fexpr.TokenIdentifier:
		...
		if macroFunc, ok := identifierMacros[token.Literal]; ok {
			...
			return &ResolverResult{
				...
				Params:     Params{placeholder: macroValue},
			}, nil
		}
	...
	case fexpr.TokenText:
		...
		return &ResolverResult{
			Identifier: "{:" + placeholder + "}",
			Params:     Params{placeholder: token.Literal},
		}, nil
	case fexpr.TokenNumber:
		...
		return &ResolverResult{
			Identifier: "{:" + placeholder + "}",
			Params:     Params{placeholder: cast.ToFloat64(token.Literal)},
		}, nil
...
func resolveEqualExpr(equal bool, left, right *ResolverResult) Expression {
	equalOp := "="
	nullEqualOp := "IS"
	concatOp := "OR"
	nullExpr := "IS NULL"
	...
	if isLeftEmpty && isRightEmpty {
		return NewExp(fmt.Sprintf("'' %s ''", equalOp), mergeParams(left.Params, right.Params))
	}
...
// mergeParams returns new Params where each provided params item
...
func wrapLikeParams(params Params) Params {
	result := Params{}
...
func (e *opExpr) Build(db *BuildContext, params Params) string {
	return e.op
}

// -------------------------------------------------------------------

var _ Expression = (*concatExpr)(nil)
...
//
// Implements [dbx.Expression] interface.
func (e *concatExpr) Build(db *BuildContext, params Params) string {
	if len(e.parts) == 0 {
		return ""
	}
...
var _ Expression = (*manyVsManyExpr)(nil)
...
//
// Implements [dbx.Expression] interface.
func (e *manyVsManyExpr) Build(db *BuildContext, params Params) string {
	...
	whereExpr, buildErr := buildResolversExpr(
		...
		&ResolverResult{
			...
			AfterBuild: Not, // inverse for the not-exist expression
		},
	)
...
var _ Expression = (*manyVsOneExpr)(nil)
...
//
// Implements [dbx.Expression] interface.
func (e *manyVsOneExpr) Build(db *BuildContext, params Params) string {
	if e.subQuery == nil {
		return "0=1"
	}
