...
func TestTokenFunctionsGeoDistanceExec(t *testing.T) {
	...
	result, err := fn(
		func() func(fexpr.Token) (*ResolverResult, error) {
			keys := []string{"lonA", "latA", "lonB", "latB"}
			index := 0
			return func(t fexpr.Token) (*ResolverResult, error) {
	...
	expected := "17.89"
	distance := fmt.Sprintf("%.2f", evalGeoDistance(result))
...
func TestTokenFunctionsStrftimeExec(t *testing.T) {
	...
	expected := "2027-06"
	if value := evalStrftime(result); value != expected {
		t.Fatalf("Expected date value %s, got %s", expected, value)
	}
}
...
func testCompareResults(t *testing.T, a, b *ResolverResult) {
	...
	if a.AfterBuild != nil {
		aAfterBuild = a.AfterBuild(NewExp("test")).Build(&BuildContext{}, a.Params)
	}
	var bAfterBuild string
	if b.AfterBuild != nil {
		bAfterBuild = b.AfterBuild(NewExp("test")).Build(&BuildContext{}, a.Params)
	}
	...
	if a.MultiMatchSubQuery != nil {
		aMultiMatchSubQuery = a.MultiMatchSubQuery.Build(&BuildContext{}, a.Params)
	}
	var bMultiMatchSubQuery string
	if b.MultiMatchSubQuery != nil {
		bMultiMatchSubQuery = b.MultiMatchSubQuery.Build(&BuildContext{}, b.Params)
	}
...
func evalGeoDistance(result *ResolverResult) float64 {
	parse := func(key string) float64 {
		for k, v := range result.Params {
			if strings.Contains(result.Identifier, "{:"+k+"}") && strings.Contains(key, k) {
				switch n := v.(type) {
				case string:
	...
	return earthRadiusKm * math.Acos(
		math.Cos(toRad(latA))*math.Cos(toRad(latB))*math.Cos(toRad(lonB)-toRad(lonA))+
			math.Sin(toRad(latA))*math.Sin(toRad(latB)),
	)
}

func evalStrftime(result *ResolverResult) string {
	format := fmt.Sprintf("%v", firstParamValue(result, "format"))
	timeValue := fmt.Sprintf("%v", firstParamValue(result, "time"))
	ts, _ := time.Parse("2006-01-02 15:04:05.999Z07:00", timeValue)
	...
	for _, key := range modifierKeys {
		modifier := fmt.Sprintf("%v", result.Params[key])
		switch {
		case strings.HasSuffix(modifier, "years"):
			n, _ := strconv.Atoi(strings.Fields(modifier)[0])
			ts = ts.AddDate(n, 0, 0)
		case strings.HasSuffix(modifier, "months"):
			n, _ := strconv.Atoi(strings.Fields(modifier)[0])
			ts = ts.AddDate(0, n, 0)
		}
	}
...
func firstParamValue(result *ResolverResult, key string) any {
	for k, v := range result.Params {
